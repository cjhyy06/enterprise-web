  ...
  <div class="container" :style="{height:sliderHeight+'px'}">
    <el-menu :default-active="activeIndex" @select="handleSelect" :background-color="appMenuBackgroundColor" :text-color="appMenuTextDefaultColor" :active-text-color="appMenuActiveTextColor">
      <template v-for="(menu, index) in menus">
        <el-menu-item v-if="!menu.children" :index="`${index+1}`">{{menu.name}}</el-menu-item>
        <el-submenu v-else :index="`${index+1}`">
          <template slot="title">{{menu.name}}</template>
          <el-menu-item v-for="(submenu, subIndex) in menu.children" :index="`${index}-${subIndex}`">{{submenu.name}}
          </el-menu-item>
        </el-submenu>
      </template>
    </el-menu>
